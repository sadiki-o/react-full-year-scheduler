...
import { IReactFullYearScheduler } from '../../utils/interfaces'
import { TDay } from '../../utils/types'
...
export const ReactFullYearScheduler: FC<IReactFullYearScheduler> = memo(function Calendar({
  ...
  const getAllDaysOfSpeficicMonth = (year: number, month: number) => {
          ...
          <thead>
            <tr>
              ...
              <th className="sticky left-0 bg-gray-200">
                {showTodayButton && (
                  <button onClick={goToToday} className="px-2 py-1 text-gray-500 hover:text-gray-800 ">
                    Today
                  </button>
                )}
              </th>
              ...
              {firstDayOfWeek === 'Sunday'
                ? new Array(7 * 5 + 2).fill(null).map((arr, index) => (
                    <WeekDayCell
                      key={index}
                      index={index}
                ...
                : new Array(7 * 6 - 5).fill(null).map((arr, index) => (
                    <WeekDayCell
                      key={index}
                      index={index}
              ...
              new Array(12)
                ...
                .map((el, index) => (
                  <MonthRow
                    key={index}
                    year={selectedYear}
                    monthDays={monthsWithDates[index].sort((a, b) => a.date.valueOf() - b.date.valueOf())}
                    monthIndex={index}
