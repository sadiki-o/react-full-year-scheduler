  ...
  return (
    <tr>
      ...
      {firstDayOfWeek === 'Sunday'
        ? new Array(5 * 7 + 2).fill(null).map((el, index) => {
            const dayIndex = index - monthDays[0].date.day() // avoid repetition

            return index >= monthDays[0].date.day() && index <= monthDays[0].date.day() + monthDays.length - 1 ? (
              <DayCell
                key={index}
        ...
        : new Array(7 * 6 - 5).fill(null).map((el, index) => {
            const preventOutOfRangeWeekIndex = monthDays[0].date.day() === 0 ? 6 : monthDays[0].date.day() - 1 // avoid repetition

            return index >= preventOutOfRangeWeekIndex && index <= preventOutOfRangeWeekIndex + monthDays.length - 1 ? (
              <DayCell
                key={index}
